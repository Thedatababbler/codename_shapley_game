#!/bin/bash
#SBATCH --job-name=ppo_native_baseline
...
#SBATCH --output=scripts/ppo_native_baseline-%j.out
#SBATCH --error=scripts/ppo_native_baseline-%j.err
#SBATCH --gpus=2
...
export RAY_TMPDIR="/scratch/zqin30/tmp/ray_verl_ppo_${SLURM_JOB_ID}"
mkdir -p "${RAY_TMPDIR}"
export RAY_DEDUP_LOGS=0
export RAY_OBJECT_STORE_MEMORY=$((24*1024*1024*1024))

model_path="${ACTOR_MODEL:-Qwen/Qwen2.5-7B-Instruct}"
rollout_data_dir="${VERL_DIR}/outputs/rollouts_ppo/${SLURM_JOB_ID}"
validation_data_dir="${VERL_DIR}/outputs/validation_ppo/${SLURM_JOB_ID}"
...
echo "=========================================="
echo "Native PPO baseline conda run"
...
"${PYTHON_BIN}" -m verl.trainer.main_ppo \
        ...
        trainer.experiment_name='qwen2.5_7b_ppo_native_baseline_2gpu' \
