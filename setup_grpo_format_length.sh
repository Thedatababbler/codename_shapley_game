#!/bin/bash
#SBATCH --job-name=grpo_format_length
...
#SBATCH --output=scripts/grpo_format_length-%j.out
#SBATCH --error=scripts/grpo_format_length-%j.err
#SBATCH --gpus=2
...
export RAY_TMPDIR="/scratch/zqin30/tmp/ray_verl_format_length_${SLURM_JOB_ID}"
mkdir -p "${RAY_TMPDIR}"
export RAY_DEDUP_LOGS=0
export RAY_OBJECT_STORE_MEMORY=$((24*1024*1024*1024))

model_path="${ACTOR_MODEL:-Qwen/Qwen2.5-7B-Instruct}"
rollout_data_dir="${VERL_DIR}/outputs/rollouts_format_length/${SLURM_JOB_ID}"
validation_data_dir="${VERL_DIR}/outputs/validation_format_length/${SLURM_JOB_ID}"
...
echo "=========================================="
echo "GRPO + format/length reward baseline conda run"
...
"${PYTHON_BIN}" -m verl.trainer.main_ppo \
        ...
        trainer.experiment_name='qwen2.5_7b_grpo_format_length_2gpu' \
